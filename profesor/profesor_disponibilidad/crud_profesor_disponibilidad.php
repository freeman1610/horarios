...
function addProfesorDisponibilidad($hour, $disponibilidad, $tipo_horario)
{
    global $conn;
    ...
    try {
        ...
        $stmt->bindParam(':profesor_id', $_SESSION['profe_id']);
...
function getProfesorDisponibilidad($tipo_horario)
{
    global $conn;
    $profesor_id = $_SESSION['profe_id'];
...
function updateProfesorDisponibilidad($hour, $disponibilidad, $tipoH)
{
    global $conn;
...
function geT()
{
    ...
    try {
        ...
        if ($rowCount == 0) {
            ...
            $stmt3->bindParam(':profesor_id', $profesorId);
...
function verificar($posicion, $disponibilidad, $tipoH)
{
    global $conn;
    try {
        ...
        hour = :posicion";
        $stmt = $conn->prepare($query);
        $stmt->bindParam(':id', $_SESSION['profe_id']);
        ...
        if ($rowCount == 0) {
            addProfesorDisponibilidad($posicion, $disponibilidad, $tipoH);
        } else {
            updateProfesorDisponibilidad($posicion, $disponibilidad, $tipoH);
        }
    } catch (PDOException $e) {
...
switch ($_POST['tipo']) {
    ...
    case 'verificar':
        $datos = $_POST['datos'];
        verificar($datos['posicion'], $datos['disponibilidad'], $datos['tipoh']);
        break;

    case 'cargar_tabla':
        getProfesorDisponibilidad($_POST['horario']);
        break;
