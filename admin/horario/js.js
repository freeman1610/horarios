...
function cargarSecciones(sede) {
    if (sede.value == '') {
        $('#selectSeccion').html('<option value="">Seleccione</option>')
        ...
        success: function (res) {
            if (res.status == 1) {
                res.datos.forEach(function (item) {
                    ...
                    let sec = document.getElementById('selectSede')
                    sec.innerHTML = sec.innerHTML + option
                })
                return true
            }
    ...
    res.horas_dia.forEach(item => {
        if (hora > 3) {
            html2 += `<tr class="tdDiurno">
                        <td data-id="${item.id}" onclick="cambiarHoraDia(${item.id}, '${item.contenido}')" style="cursor:pointer;" class="bg-success text-white">${item.contenido}</td>
...
function cambiarHoraDia(id, contenido) {
    Swal.fire({
        title: 'Actualizar Hora ' + contenido,
        html:
            `<div class="container">
                <div class="row">
                    <div class="col-12">
                        <div class="form-floating mb-3">
                            <input autocomplete="off" value="${contenido}" id="txtHoras" type="text" class="form-control" placeholder="">
                            <label for="txtHoras">Contenido</label>
                        </div>
                    </div>
                </div>
            </div>`,
        ...
        preConfirm: () => {
            ...
            actualizarHoraDiaAjax(id, $('#txtHoras').val());
        }
    });
    function actualizarHoraDiaAjax(id, conte) {
        $.ajax({
            type: "POST",
            url: "crud_horario.php",
            data: {
                tipo: 'update_hora_dia',
                id: id,
                contenido: conte,
            },
            success: function () {
...
function cargarContenido() {
    ...
    if (sede != '' && seccion != '' && tipoHorario != '' && semana == '') {
        cargarSemanas()
        return true
    }
                            ...
                            <div class="row mt-3" style="max-height: 100px;">
                                ...
                                <div class="col-9">
                                    <div class="input-group">
                                        ...
                                        <button id="btnAnadirExc" class="btn btn-primary d-none" onclick="excepcionProfe(${datos.posicion})">
                                            Añadir Excepción
                                        </button>
                                    </div>
                                </div>
