...
// Agregar un horario
function addHorario($pensum_id, $profesor_id, $aula_id, $hour, $seccion_id, $tipo_horario, $sede_id, $semana_inicio, $semana_fin, $horas_consecutivas, $excepcion, $horaFirstTDid, $diaText)
{
    global $conn;
...
function cargarHorario($seccion_id, $tipo_horario, $semana, $sede_id)
{
    global $conn;

    try {
        $query1 = "SELECT horario.id AS horario_id, horario.pensum_id, horario.user_id, horario.profesor_id, horario.aula_id, 
...
function cargarProfesorPerfilProfesional($hour, $tipo_horario, $update, $profesor_id, $semana, $sede_id, $pensum_id, $hor_id)
{
    global $conn;


    // Consulta 1: Obtener profesores disponibles en la hora y tipo de horario específicos
    ...
    if ($rowCount == 0) {
        if ($update == 'true') {
            $queryProfes = "SELECT profesor.id, profesor.name, profesor.last_name, profesor.cedula 
                            ...
                            AND horario.pensum_id = $pensum_id";
...
function showDataFormPensumAulas($seccion_id, $update, $horario_id, $hour, $tipo_horario, $semana, $sede_id)
{
    global $conn;
...
function dragHorario($data_origen, $data_destino, $tipo_horario)
{
    ...
    try {
        ...
        if ($data_origen['update'] == 'true' && $data_destino['update'] == 'true') {
            ...
            $stmt1->bindParam(':user_id', $data_destino['user_id']);
            $stmt1->bindParam(':hour', $data_destino['posicion']);
            ...
            $stmt2->bindParam(':user_id', $data_origen['user_id']);
            $stmt2->bindParam(':hour', $data_origen['posicion']);
...
function verDisponibilidadProfe($texto, $sede_id)
{
    global $conn;
...
function traerTodosProfesoresExcepcion($sede_id, $hour, $tipo_horario)
{
    global $conn;
...
function updateHoraDia($id, $contenido)
{
    global $conn;
    try {
        $query = "UPDATE horas_dia 
                    SET 
                        contenido = '$contenido'
                    WHERE 
                        id = $id";
        ...
        $response = ['Error updateHoraDia: ' . $e];
        // Convertir el array en formato JSON
        $jsonResponse = json_encode($response);
        http_response_code(404);
        // Imprimir el JSON como respuesta
...
switch ($_POST['tipo']) {
    case 'cargarHorario':
        cargarHorario($_POST['seccion_id'], $_POST['tipo_horario'], $_POST['semana'], $_POST['sede_id']);
        break;
    ...
    case 'cargarSelectSede':
        ...
        break;
    case 'crear':
        addHorario($_POST['pensum_id'], $_POST['profesor_id'], $_POST['aula_id'], $_POST['hour'], $_POST['seccion_id'], $_POST['tipo_horario'], $_POST['sede_id'], $_POST['semana_inicio'], $_POST['semana_fin'], $_POST['horas_consecutivas'], $_POST['excepcion'], $_POST['horaFirstTDid'], $_POST['diaText']);
        break;
    case 'drag':
    ...
    case 'verDisponibilidadProfe':
        verDisponibilidadProfe($_POST['boton'], $_POST['sede_id']);
        break;
    case 'desbloquearDispoProfe':
        desbloquearDispoProfe($_POST['id']);
        break;
    case 'showDataFormPensumAulas':
        showDataFormPensumAulas($_POST['seccion_id'], $_POST['update'], $_POST['horario_id'], $_POST['hour'], $_POST['tipo_horario'], $_POST['semana'], $_POST['sede_id']);
        break;
    case 'cargarProfesorPerfilProfesional':
        cargarProfesorPerfilProfesional($_POST['hour'], $_POST['tipo_horario'], $_POST['update'], $_POST['profesor_id'], $_POST['semana'], $_POST['sede_id'], $_POST['pensum_id'], $_POST['hor_id']);
        break;
    case 'traerTodosProfesoresExcepcion':
        traerTodosProfesoresExcepcion($_POST['sede_id'], $_POST['hour'], $_POST['tipo_horario']);
        break;
    case 'update_hora_dia':
        updateHoraDia($_POST['id'], $_POST['contenido']);
        break;
}
