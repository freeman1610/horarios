$(document).ready(function () {
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.status == 0) {
                $('#divSave').html('<button id="btnGuardar" onclick="guardarDatos(' + id + ')" class="btn btn-success">Guardar</button>')
                $('#h3Status').html('Estado: <span class="text-danger">Sin Crear</span>')
                $('#statuss').val(0)
            }
            if (res.status == 1) {
                $('#h3Status').html('Estado: <span class="text-success">Guardado</span>')
                $('#divSave').html('<button id="btnGuardar" onclick="desbloquear(' + id + ')" class="btn btn-success">Desbloquear</button>')
                $('#statuss').val(1)
            }
                    ...
                    if (matchingData1.disponibilidad) {
                        ...
                        if (matchingData2.length > 0) {
                            ...
                            matchingData2.forEach(function (data) {
                                buttonContent += '<div class="p-2 mt-2" style="background:' + coloresSuaves[o] + '"> Carrera: ' + data.carrera +
...
function guardarDatos(id) {
    Swal.fire({
        ...
        text: 'Una vez guarda no podra modificarse hasta que un Coordinador lo vuelva a desbloquearlo',
        preConfirm: () => {
            $.ajax({
                url: '../profesor_disponibilidad/crud_profesor_disponibilidad.php',
                method: "POST",
                data: { tipo: 'guardar_carga_horaria', profesor_id: id },
                success: function (res) {
...
function desbloquear(id) {
    Swal.fire({
        ...
        cancelButtonText: 'No',
        icon: 'warning',
        title: '¿Estas seguro de Desbloquear la Disponibilidad Horaria?',
        preConfirm: () => {
            $.ajax({
                url: '../profesor_disponibilidad/crud_profesor_disponibilidad.php',
                method: "POST",
                data: { tipo: 'desbloquear_carga_horaria', profesor_id: id },
                success: function (res) {
                    res.status == 0 ? $('#statuss').val(0) : ''
                    $('#h3Status').html('Estado: <span class="text-warning">Sin Cargar</span>')
                    $('#divSave').html('<button id="btnGuardar" onclick="guardarDatos(' + id + ')" class="btn btn-success">Guardar</button>')
                    cargarTabla()
                    Swal.fire({
                        ...
                        showConfirmButton: false,
                        icon: 'success',
                        title: 'Disponibilidad Horaria Desbloqueada'
                    })
                },
                error: function (error) {
                    console.error(error);
